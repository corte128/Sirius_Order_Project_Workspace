...
if (session.getAttribute("__default_endpoint__") == null) {
    session.setAttribute("__default_endpoint__", sampleSuperadminProxyid._getDescriptor().getEndpoint());
}
if (submitAction != null && submitAction.equals("Update")) {
    sampleSuperadminProxyid._getDescriptor().setEndpoint(request.getParameter("endpoint"));
}
else if (submitAction != null && submitAction.equals("Reset")) {
    sampleSuperadminProxyid._getDescriptor().setEndpoint((String) session.getAttribute("__default_endpoint__"));
}
String endpoint = sampleSuperadminProxyid._getDescriptor().getEndpoint();

boolean hasAsync = com.ibm.ccl.ws.jaxws.gstc.util.AsyncUtils.hasAsync(sampleSuperadminProxyid.getClass());
